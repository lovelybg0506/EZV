...
<%
	...
	LBoardBean bean=bdao.getOneBoard(num);
%>
<script type="text/javascript">
	function event(num){
		if(confirm("정말 삭제하시겠습니까?")){
			location.href="lboardDeleteProc.jsp?num="+num
		}else{
			return;
		}
	}
										...
										<table width="800" class="line">
											<tr height="80" class="line">
												<td align="left" colspan="2"><span class="subject"><%=bean.getSubject() %></span><br>
													지점 &nbsp;<%=bean.getPlace() %> &nbsp;|&nbsp; 등록일 &nbsp;<%=bean.getReg_date() %>
												</td>
											</tr>
					            ...
					            <div class="action take-tour" align="center">
			                        <div width="800" align="center">
											...
											<input type="button" value="답글쓰기"
     								   		onclick="location.href='lboardReWriteForm.jsp?num=<%=bean.getNum()%>&ref=<%=bean.getRef() %>&re_step=<%=bean.getRe_step()%>&re_level=<%=bean.getRe_level()%>'">                                    	
                                    	</c:if>
									<input type="button" value="수정하기" onclick="location.href='lboardUpdateForm.jsp?num=<%=bean.getNum()%>'">
									<input type="button" value="삭제하기" onclick="location.href='lboardDeleteProc.jsp?num=<%=bean.getNum()%>'">
									<input type="button" value="목록보기" onclick="location.href='service_LnF.jsp?num=<%=bean.getNum()%>'">
			                        </div>
			                    </div>
                            </div>
                         </div>
