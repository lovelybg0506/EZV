...
public class QBoardDAO {
	...
	public void insertBoard(QBoardBean bean) {
		getCon();
		...
		try {
			...
			String refsql="select max(ref) from qboard";
			// 쿼리 실행 객체
			...
			String sql="insert into qboard values(qboard_seq.nextval,?,?,?,?,sysdate,?,?,?,?)";
	...
	public Vector<QBoardBean> getAllBoard(int start,int end){ 
		
	    //public Vector<BoardBean> getAllBoard() { //객체가 없는 메서드
			
			// 리턴할 객체 선언
			Vector<QBoardBean> vec=new Vector<>();
			getCon();
			
			try {
				// 쿼리 준비
				String sql="select * from (select A.*,Rownum Rnum from(select * from qboard order by ref desc,re_step asc)A)"
				          +"where Rnum >= ? and Rnum <= ?"; 
				...
				while(rs.next()) {
					QBoardBean bean=new QBoardBean();
	...
	//id를 가져와서 게시판 보여주기
	public Vector<QBoardBean> ugetAllBoard(String id,int start,int end){ 
    ...
    //public Vector<LBoardBean> getAllBoard() { //객체가 없는 메서드
		...
		try {
			// 쿼리 준비
			String sql="select * from (select A.*,Rownum Rnum from(select * from qboard where id= ? order by ref desc,re_step asc)A)"
			          +"where Rnum >= ? and Rnum <= ?"; 
			// 모르겠으면 이거 그대로 대입시키면 된다. 공부해보자 , Rownum은 여기서조회되는 정보에 번호를 맥인다==>자동번호를 붙인다
			// String sql="select * from board order by ref desc,re_step asc";
	...
	public QBoardBean getOneBoard(int num) {
		// 리턴타입 선언 
		QBoardBean bean=new QBoardBean();
		getCon();
		...
		try {
			...
			// 쿼리 준비
			String sql="select * from qboard where num=?";
	...
	// 답변글이 저장되는 메서드
	public void reWriteBoard(QBoardBean bean) {
		...
		try {
			...
			String levelsql="update qboard set re_step=re_step+1 where ref=? and re_step > ?";
			// 쿼리 삽입 객체 선언
	...
	public QBoardBean getOneUpdateBoard(int num) {
		// 리턴타입 선언
		QBoardBean bean=new QBoardBean();
		getCon();
	...
	public String getPass(int num) {
		...
		try {
			// 쿼리 준비
			String sql="select password from qboard where num=?";
			// 쿼리 실행할 객체 선언
			pstmt=conn.prepareStatement(sql);
			pstmt.setInt(1,num);
			// 쿼리 실행후 결과를 리턴
	...
	// 하나의 게시글을 수정하는 메서드
	public void updateBoard(QBoardBean bean) {
		getCon();
		
		try {
			// 쿼리 준비
			String sql="update qboard set content=? where num=?";
	...
	public void deleteBoard(int num) {
		...
		try {
			// 쿼리 준비
			String sql="delete from qboard where num=?";
			// 쿼리 실행할 객체 선언
			pstmt=conn.prepareStatement(sql);
			// ?
			pstmt.setInt(1, num);
	...
	public int ugetAllCount(String id) {
		...
		try {
			// 쿼리 준비
			String sql="select count(*) from qboard where id=?";
			// 쿼리 실행할 객체 선언
			pstmt=conn.prepareStatement(sql);
			pstmt.setString(1, id);
			// 쿼리 실행후 결과 리턴
